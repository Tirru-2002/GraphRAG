            ...
            if token.pos_ == "VERB":
                ...
                if subject and obj:
                    ...
                    relationships.append({
                        ...
                        'type': self._classify_relationship(token.text), ## every text will here is verb, because token.pos_ == "VERB"
                        'verb': token.text
                    })
                ...
                if relationships:
                    ...
                    for rtype, rels in rel_type_map.items():
                        ...
                        for (src_lbl, tgt_lbl), pair_rels in pair_map.items():
                            ...
                            # 2) Fallback: label-qualified MATCH + MERGE relationship (safer than unlabeled)  --> best practice
                            try:
                                fallback_query = f"""
                                UNWIND $pairs AS rel
